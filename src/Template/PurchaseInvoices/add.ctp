...
<div class="portlet solid" style="background-color:#FFF;">
	<div style="margin-bottom:3px;">
		<span class="label label-sm" style="font-size: 14px;font-weight: bold;background-color: #2b8c2b;">Purchase Invoice No. : 1 </span>
	</div>
	...
	$(document).ready(function() {
		...
		function renameRows(){
			...
			$('.mainTable tbody.mainTbody tr.mainTr').each(function(){
				...
				$(this).find('td:nth-child(2) input').attr({name:'purchase_invoice_rows['+i+'][quantity]', id:'purchase_invoice_rows-'+i+'-quantity'});
				$(this).find('td:nth-child(3) input').attr({name:'purchase_invoice_rows['+i+'][rate]', id:'purchase_invoice_rows-'+i+'-rate'});
				$(this).find('td:nth-child(4) input').attr({name:'purchase_invoice_rows['+i+'][amount]', id:'purchase_invoice_rows-'+i+'-amount'});
				$(this).find('td:nth-child(5) input').attr({name:'purchase_invoice_rows['+i+'][discount_percentage]', id:'purchase_invoice_rows-'+i+'-discount_percentage'});
				$(this).find('td:nth-child(6) input').attr({name:'purchase_invoice_rows['+i+'][discount_amount]', id:'purchase_invoice_rows-'+i+'-discount_amount'});
